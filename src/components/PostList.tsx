...
async function PostList({ subject }: { subject?: Subject }) {
  ...
  // This is where I check for a subject for if the subject is present for the filtered page
  if (subject) {
    posts = posts.filter((post) => post.frontMatter.topics.includes(subject));
  }
  // This is where I filter for isDraft - anything in draft will be filtered out
  posts = posts.filter((post) => !post.frontMatter.isDraft);
  return (
